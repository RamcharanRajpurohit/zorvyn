...
const record_model_1 = require("../records/record.model");
...
async function getOverview(query) {
    ...
    const groupId = query.trend === 'daily'
        ? {
    ...
    const trendProjection = query.trend === 'daily'
        ? {
            label: {
    ...
    const [totals, categories, trends, recentActivity] = await Promise.all([
        ...
        record_model_1.RecordModel.find(baseMatch)
    ...
    const mappedRecentActivity = recentActivity.map((item) => {
        const createdBy = item.createdBy && typeof item.createdBy === 'object' && '_id' in item.createdBy
            ? {
