...
function buildMonthlyDistribution(totalEntries: number) {
...
async function ensureDemoUsers() {
  ...
  const admin = await UserModel.findOne({ email: 'admin@walletwhiz.dev' })
  const analyst = await UserModel.findOne({ email: 'analyst@walletwhiz.dev' })
  const viewer = await UserModel.findOne({ email: 'viewer@walletwhiz.dev' })
...
async function run() {
  ...
  const currentCount = await RecordModel.countDocuments({ isDeleted: false })
  ...
  for (let index = 0; index < docs.length; index += BATCH_SIZE) {
    ...
    await RecordModel.insertMany(batch, { ordered: false })
    inserted += batch.length
    process.stdout.write(`\rInserted ${inserted} / ${docs.length}`)
  }

  const finalCount = await RecordModel.countDocuments({ isDeleted: false })
